...
public class Ex1 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("원본 물품 = " + items);

        Collections.sort(items);
        System.out.println("정렬된 물품 = " + items);

        Collections.reverse(items);
        System.out.println("역순 정렬 물품 = " + items);

        Collections.shuffle(items);
        System.out.println("섞은 물품 = " + items);

        Collections.addAll(items, "휴대폰", "카메라", "책");
        System.out.println("추가 후 물품 = " + items);

        int frequency = Collections.frequency(items, "책");
        System.out.println("책의 개수 = " + frequency);

        items = new ArrayList<>(new HashSet<>(items));
        System.out.println("중복 제거 = " + items);

        String maxItem = Collections.max(items);
